...
**Learning:** Using `TF_PLUGIN_CACHE_DIR` with `terragrunt run-all` can cause race conditions and corrupt installations during concurrent provider downloads.
**Action:** Use Terragrunt's native provider cache (`TERRAGRUNT_PROVIDER_CACHE=1` and `TERRAGRUNT_PROVIDER_CACHE_DIR`) instead, which correctly handles concurrent downloads across multiple modules.
