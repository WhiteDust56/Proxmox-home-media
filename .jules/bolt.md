...
## 2024-06-13 - Optimize Provider Caching for Terragrunt
**Learning:** Using Terraform's `TF_PLUGIN_CACHE_DIR` with Terragrunt can still lead to race conditions or corrupt installations during concurrent provider downloads across multiple modules.
**Action:** Use Terragrunt's native provider cache by setting `TERRAGRUNT_PROVIDER_CACHE=1` and configuring `TERRAGRUNT_PROVIDER_CACHE_DIR` to a workspace path to ensure reliable and fast concurrent execution.
