...
## 2024-06-18 - Concurrent Provider Downloads in Terragrunt
**Learning:** Using `TF_PLUGIN_CACHE_DIR` with Terragrunt causes race conditions and corrupt provider installations when multiple modules concurrently download providers.
**Action:** Use Terragrunt's native provider cache (`TERRAGRUNT_PROVIDER_CACHE=1` and `TERRAGRUNT_PROVIDER_CACHE_DIR`) instead of `TF_PLUGIN_CACHE_DIR` to prevent redundant downloads and race conditions safely.